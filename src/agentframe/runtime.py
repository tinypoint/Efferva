...
class CodexRuntime:
    ...
    async def start(self) -> None:
        async with self._start_lock:
            ...
            environment["AGENTFRAME_DATABASE_URL"] = self._database_url
    ...
    def _runtime_command(self) -> list[str]:
        ...
        if self._openai_base_url:
            provider = "agentframe_proxy"
            provider_overrides = (
                ("model_provider", provider),
                (f"model_providers.{provider}.name", "AgentFrame LLM proxy"),
            ...
            await self._write(
                {
                    ...
                    "error": {
                        "code": -32601,
                        "message": f"AgentFrame cannot handle server request {message['method']}",
                    },
                }
            )
    ...
    def _fail_subscribers(self, error: Exception) -> None:
        for thread_id, queue in self._thread_queues.items():
            queue.put_nowait(
                {
                    "method": "agentframe/runtimeError",
                    "params": {
                        "threadId": thread_id,
                        "message": str(error),
                    },
